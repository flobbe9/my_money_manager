...
import { equalsIgnoreCaseTrim, isBlank, log, logApiResponse, logError, logWarn } from "./basicUtils";
import { ApiExceptionFormat } from './../abstract/ApiExceptionFormat';
import CsrfToken from "../abstract/CsrfToken";
...
async function getFetchHeaders(url: string, headers?: Record<string, any>, refreshCsrfToken = false, method = "get") {
    ...
    const csrfToken = chooseCsrfTokenByUrl(url);
...
/**
 * Get correct {@link CsrfToken} for given url stored in globalVariables.tsx.
 * 
 * @param url that needs a csrf token
 * @returns the {@link CsrfToken} stored locally related to given url or an empty instance (of no use but still not null :))
 */
function chooseCsrfTokenByUrl(url: string): CsrfToken {
    
    // if (url.startsWith(BACKEND_BASE_URL))
    //     return CSRF_TOKEN_DOCUMENT_BUILDER;

    // if (url.startsWith(USER_SERVICE_BASE_URL))
    //     return CSRF_TOKEN_USER_SERVICE;

    logWarn("Failed to find local csrf token by url " + url + ". Returning an empty instance instead.");
...
export async function fetchCsrfToken(url: string): Promise<string> {
    ...
    if (isBlank(BACKEND_BASE_URL) || !isCsrfTokenFree(url))
        return "";

    const response = await fetchAny(BACKEND_BASE_URL + "/getCsrfToken");
...
function isCsrfTokenFree(url: string) {
    ...
    return url.startsWith(BACKEND_BASE_URL);
}